...
function addNewBlockToHTML(bookId, parId) {
  ...
  output +=
    ...
    "_" +
    (parId - 1) +
    '" class="par_heading"></div>';

  //word by word translate block
  //内部可以包含多个wbw块
  output +=
    '	<div id="wblock_' + bookId + "_" + (parId - 1) + '" class="wbwdiv"></div>';

  //translate div
  output +=
    '	<div id="tran_' + bookId + "_" + (parId - 1) + '" class="trandiv"></div>';
  ...
  //note div
  output +=
    '	<div id="note_' + bookId + "_" + (parId - 1) + '" class="pnotediv"></div>';
...
function renderToc(bookId, parBegin, parEnd) {
  ...
  for (var iPar = parBegin; iPar < parEnd; iPar++) {
    ...
    if (parHeadingLevel > 0) {
      output +=
        '<li class="toc_h_' + (parHeadingLevel - firstHeadingLevel + 1) + '">';
      output +=
        '<a href="#par-' + bookId + "-" + iPar + '">' + parTitle + "</a>";
      output += "</li>";
    }
  }
  output += "</ul>";
...
function updataWordParContainer(bookId, parId) {
  document.getElementById(
    "wblock-" + bookId + "-" + parId + '"'
  ).innerHTML = renderWordParContainerInner(bookId, parId);
}
    ...
    if (type == "heading") {
      ...
      //if(level>0)
      {
        ...
        /*
				...
				*/
        let tocText =
          '<p class="toc_item_' + level + " " + language + '_text">';
        tocText +=
          "<a onclick=\"editor_goto_link('" +
          bookId +
          "'," +
...
function insertBlockToHtml(element) {
  ...
  switch (type) {
    case "wbw":
      ...
      $("#wnote_" + bookId + "_" + (paragraph - 1)).html(
        renderNoteShell(element)
      );
    ...
    case "translate":
      ...
      //逐句
      if (_display_sbs == 1) {
        let eAllSent = document
          .getElementById(blockId)
          .getElementsByClassName("tran_sent");
        for (let iSen = 0; iSen < eAllSent.length; iSen++) {
          ...
          if (eSBSDiv) {
            let eBlockSenDiv = document.getElementById(
              "sent_" + senA + "_" + blockId
            );
            if (!eBlockSenDiv) {
              //没有 添加
  ...
  if (block != null) {
    ...
    if (sent != null) {
      for (const element of sent) {
        let eText = document.getElementById(
          "ta_" + blockId + "_" + element.begin + "_" + element.end
        );
        if (eText) {
...
function renderTranslateParBlockInner(elementBlock) {
  ...
  for (iSen = 0; iSen < allSen.length; iSen++) {
    ...
    output +=
      "<div id='tran_pre_" + bId + "_" + senA + "' class='tran_sent_pre'>";
    if (senText == "") {
      output +=
        "<span style='color:gray;'>" + gLocal.gui.translation + "</span>";
    } else {
      output += term_std_str_to_tran(senText);
    }
    output += "</div>";
    output +=
      "<div id='tran_sent_text_div_" +
      bId +
      "_" +
      senA +
      "' class='tran_sen_textarea'";
    //在非编辑状态下 不显示 编辑框
    ...
    if (power != "read") {
      ...
      if (readonly != 1) {
        ...
        output +=
          "<textarea id='ta_" + bId + "_" + senBegin + "_" + senEnd + "' ";
        output +=
...
function getSuperTranslateModifyString(inString, par_num, par_guid, language) {
  ...
  if (par_num_last >= arrString.length - 1) {
    ...
    for (var x = 0; x < arrString.length; x++) {
      ...
      var language_type = translate_super_split(arrString[x], language)
        .language_type;
      var output_super = "";
      for (var super_x = 0; super_x < superString.length; super_x++) {
        ...
        output_super +=
          superStringPosition_End + "_" + superStringPosition_Start;
        output_super += "</span>";
        output_super +=
          "<span id='SBS_senText_" +
          par_guid +
          "_" +
          superStringPosition_End +
          '\' class="tooltip">';
      ...
      var output_super_0 =
        "<div class=" + language_type + "_text>" + output_super + "</div>";
      if (document.getElementById(sent_ID).innerHTML.length != 0) {
        if (
          document.getElementById(sent_ID).innerHTML == gLocal.gui.sent_trans
        ) {
  ...
  switch (updown) {
    case "up_par":
      ...
      new_prev_super_info.senText =
        ...
        curr_super_info.senText.replace(/#/g, "").slice(0, superStringPosition);
      var new_senText_array = translate_super_split(
        new_prev_super_info.senText,
        language
      );
      new_prev_super_info.senA = "0_";
      for (new_senText_array_i in new_senText_array) {
        new_prev_super_info.senA +=
          new_senText_array[new_senText_array_i].length;
        new_prev_super_info.senA +=
          "#" + new_senText_array[new_senText_array_i].length + "_";
      ...
      new_prev_super_info.senA = (new_prev_super_info.senA + "#").replace(
        /##/g,
        ""
      );

      new_curr_super_info.GUID = curr_super_info.GUID;
      new_curr_super_info.senText = curr_super_info.senText
        .replace(/#/g, "")
        .slice(superStringPosition);
      var new_senText_array = translate_super_split(
        new_curr_super_info.senText,
        language
      );
      ...
      new_curr_super_info.senA = (new_curr_super_info.senA + "#").replace(
        /##/g,
        ""
      );
...
function getPrevNextTrans_Guid(direction, par_guid) {
  ...
  if (elementBlock.getElementsByTagName("sen").length != 0) {
    for (var super_iData in super_arr_xmlParData) {
      ...
      for (
        var super_iSen = 0;
        super_iSen < super_arr_xmlParData[super_iData].childNodes.length;
        super_iSen++
      ) {
        var super_xmlParData_Detail =
          super_arr_xmlParData[super_iData].childNodes[super_iSen];
        super_xmlParAll_Sen +=
          super_xmlParData_Detail.getElementsByTagName("text")[0].innerHTML +
          "#";
        super_xmlParAll_Loc +=
          super_xmlParData_Detail.getElementsByTagName("a")[0].innerHTML + "#";
      }
    ...
    for (
      var super_iBlock = 0;
      super_iBlock < super_arr_xmlParInfo.length;
      super_iBlock++
    ) {
      if (super_iBlock == "length") {
        break;
      ...
      if (super_GUID == par_guid && super_iBlock == 0) {
        ...
        next_super_info.GUID = getNodeText(
          super_arr_xmlParInfo[super_iBlock + 1],
          "id"
        ); //得到後一個guid
        ...
        super_GUID == par_guid &&
        super_iBlock > 0 &&
        super_iBlock < super_arr_xmlParAllsen.length - 1
      ) {
        prev_super_info.GUID = getNodeText(
          super_arr_xmlParInfo[super_iBlock - 1],
          "id"
        ); //得到前一個guid
        next_super_info.GUID = getNodeText(
  ...
  switch (language_type) {
    case "ZH":
      for (var i_cntransplit in cn_transplit) {
        newString = inString;
        eval(
          "newString = newString.replace(" + cn_transplit[i_cntransplit] + ")"
        );
      }
    ...
    case "EN":
      ...
      for (var i_entransplit in en_transplit) {
        newString = inString;
        eval(
          "newString = newString.replace(" + en_transplit[i_entransplit] + ")"
        );
      }
...
function renderWordParBlockInner(elementBlock) {
  ...
  for (let iWord = 0; iWord < allWord.length; iWord++) {
    ...
    //word div class
    let strMouseEvent =
      ' onmouseover="on_word_mouse_enter()" onmouseout="on_word_mouse_leave()"';
    if (wType == ".ctl." && wGramma == ".a.") {
      ...
      if (wReal == "") {
        output +=
          '<div id="wb' +
          wID +
          '" class="word_punc un_parent sent_gramma_' +
          (sent_gramma_i % 3) +
          '" >'; //符號
      } else {
        if (wType == ".un.") {
          output +=
            '<div id="wb' +
            wID +
            '" class="word un_parent sent_gramma_' +
            (sent_gramma_i % 3) +
            '" >'; //粘音詞
        } else if (wType == ".comp.") {
          output +=
            '<div id="wb' +
            wID +
            '" class="word comp_parent sent_gramma_' +
            (sent_gramma_i % 3) +
            '" >'; //複合詞
        } else {
          output +=
...
function magic_sentence_cut() {
  ...
  for (i_magic = 0; i_magic < all_sent_array.length; i_magic++) {
    if (
      all_sent_array[i_magic].getElementsByClassName("word sent_gramma_1")
        .length != 0
    ) {
      all_sent_array[i_magic].getElementsByClassName(
        "word sent_gramma_1"
      )[0].className += " sent_cut";
    }
    if (
      all_sent_array[i_magic].getElementsByClassName("word sent_gramma_2")
        .length != 0
    ) {
      all_sent_array[i_magic].getElementsByClassName(
        "word sent_gramma_2"
...
function renderWordHeadInner(element) {
  ...
  if (sign_count != 0 && letter_count == 0) {
    ...
    output +=
      "<a name='w" + wId + "' onclick='on_word_click(\"" + wId + "\")'>";
  }
  if (wpali)
    output +=
      '<span id="whead1_' + wid + '" class="whead paliword1 ' + wStyle + '">';
  output += wpali;
  output += "</span>";
  output += "</a>";
  if (wNote.length > 0) {
  ...
  if (newMsg > 0) {
    output +=
      "<span class='word_msg' onclick=\"word_msg_counter_click('" +
      wid +
      "')\">" +
      newMsg +
      "</span>";
  ...
  output +=
    '<span id="whead2_' + wid + '" class="whead paliword2 ' + wStyle + '">';
  output += "</span>";
  return output;
}
...
function renderWordDetailByElement_edit_a(xmlElement) {
  ...
  //编辑模式开始
  {
      ...
      if (thisWord == g_DictWordList[iDict].Pali) {
        if (
          (g_DictWordList[iDict].Type != "" &&
            g_DictWordList[iDict].Type != "?") ||
          (g_DictWordList[iDict].Gramma != "" &&
            g_DictWordList[iDict].Gramma != "?")
        ) {
          var arrCase =
            g_DictWordList[iDict].Type + "#" + g_DictWordList[iDict].Gramma;
    ...
    if (sReal.length < 4) {
      ...
      currMeaning = getLocalFormulaStr(
        currGramma,
        cutString(currMeaning, sReal.length * 6)
      );
    }

    renderMeaning = currMeaning.replace(/{/g, "<span class='grm_add_mean'>");
    renderMeaning = renderMeaning.replace(/}/g, "</span>");
    renderMeaning = renderMeaning.replace(
      /\[/g,
      "<span class='grm_add_mean_user'>"
    );
    renderMeaning = renderMeaning.replace(/\]/g, "</span>");
    //格位公式结束
    if (sMean.length == 0) {
    ...
    _txtOutDetail += "</p>";
    _txtOutDetail +=
      "<div id='mean_" + wordID + '\' class="case_dropdown-content">';
    _txtOutDetail += "</div>";
    _txtOutDetail += "</div>";
    ...
    _txtOutDetail += '<div class="case_dropdown">';
    _txtOutDetail +=
      "<svg class='edit_icon';'><use xlink:href='svg/icon.svg#ic_more'></use></svg>";
    ...
    _txtOutDetail += "<p class='case_dropbtn' >";
    sOm = sOm.replace(
      "_un_auto_factormean_",
      getLocalGrammaStr("_un_auto_factormean_")
    );
    ...
    if (sOm == "?" || sOm.substring(0, 3) == "[a]") {
      ...
      for (iFactor in listFactorForFactorMean) {
        currDefualtFM +=
          findFirstMeanInDict(listFactorForFactorMean[iFactor]) + "+"; //拆分元素加号分隔
      }
    ...
    if (mType == ".un." || mType == ".comp.") {
      ...
      if (nextElement != null) {
        ...
        if (getNodeText(nextElement, "un") == sId) {
          //若有孩子則显示收起按鈕
          _txtOutDetail +=
            "<button class='in_word_button' onclick='edit_un_remove(\"" +
            wordID +
            "\")'>";
          _txtOutDetail +=
          ...
          //無kid展開按鈕
          _txtOutDetail +=
            "<button class='in_word_button' onclick='edit_un_split(\"" +
            wordID +
            "\")'>";
          _txtOutDetail +=
...
function renderWordDetailByElement(xmlElement) {
  ...
  if (g_useMode == "edit") {
      ...
      if (
        ...
        if (sCase == tempCase && g_DictWordList[iDict].Mean.length > 0) {
          if (currGrammaMeaning == "") {
            if (
              dict_language_enable.indexOf(g_DictWordList[iDict].Language) >= 0
            ) {
        ...
        if (
          ...
          for (var i = 0; i < arrMean.length; i++) {
            if (arrMean[i].length > 0 && arrMean[i] != "?") {
              ...
              if (
                dict_language_enable.indexOf(g_DictWordList[iDict].Language) >=
                0
              ) {
                arrMeaning.push(
                  g_DictWordList[iDict].dictID +
    ...
    sMean = sMean.replace(
      "_un_auto_mean_",
      getLocalGrammaStr("_un_auto_mean_")
    );
    ...
    if (sReal.length < 4) {
      ...
      currMeaning = getLocalFormulaStr(
        currGramma,
        cutString(orgMeaning, sReal.length * 6)
      );
    }
    ...
    _txtOutDetail +=
      "<a style='z-index:250; position:absolute; margin-right:2em;'>";
...
function refresh_Dispaly_Cap() {
  ...
  for (refresh_i in allTextLen_array) {
    gNewArrayDocParagraph.push(
      allTextLen_array[refresh_i].length / gtext_max_length
    );
  }
}
function prev_page() {
...
function render_word_mean_menu2(spell) {
  ...
  for (var iWord in sWord) {
    ...
    output +=
      '<span style="z-index:220" class="case_dropdown-title" onclick="submenu_show_detail(this)">';
    output +=
      '<svg class="icon" style="fill:var(--main-color)"><use xlink:href="svg/icon.svg#ic_add"></use></svg>';
    ...
    for (var i in partList2) {
      var L_width = getLocalDictname(partList2[i].mean).replace(
        /[\u0391-\uFFE5]/g,
        "aa"
      ).length;
    ...
    for (var i in partList2) {
      ...
      output +=
        ...
        "_" +
        i +
        "' style='margin-left: 0.4em; display:flex'>";
      ...
      output +=
        "</div><div style='width:15em; display:flex; flex-wrap: wrap;'>" +
        htmlMean +
        "</div>";
      output += "</a>";
    }
...
function render_word_mean_menu(xElement) {
  ...
  for (var iWord in sWord) {
    ...
    for (var i in partList2) {
      ...
      output += "</div>";
      output +=
        "<div style='width:15em; display:flex; flex-wrap: wrap;'>" +
        htmlMean +
        "</div>";
      output += "</a>";
    }
...
function render_get_word_parent_list(sWord) {
  ...
  for (var x in g_DictWordList) {
    if (g_DictWordList[x].Pali == wReal) {
      {
        if (
          g_DictWordList[x].Parent != "" &&
          g_DictWordList[x].Parent != sWord
        ) {
...
function repeat_combine(list) {
  ...
  for (
    var repeat_combine_i = 0;
    repeat_combine_i < list.length;
    repeat_combine_i++
  ) {
    ...
    for (
      repeat_combine_j = repeat_combine_i + 1;
      repeat_combine_j < list.length;
      repeat_combine_j++
    ) {
      if (
        ...
        repeat_string.mean =
          repeat_string.mean + "$" + list[repeat_combine_j].mean;
...
function refreshNoteNumber() {
  $("wnh").each(function (index, element) {
    ...
    $(this).html(
      "<span onclick=\"show_pop_note('" + id + "')\">" + (index + 1) + "</span>"
    );
  });
