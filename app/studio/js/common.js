      ...
      switch (item) {
        case "begin":
          if (getNodeText(xAllWord[iWordIndex], "case") == ".un:begin.") {
            ...
            document.getElementById(wordid).style.display = isVisible
              ? "block"
              : "none";
          }
          break;
  ...
  if (_display_sbs != mode) {
    ...
    if (_display_sbs == 1) {
      ...
      for (var iSen = 0; iSen < eAllSent.length; iSen++) {
        ...
        if (eSBSDiv) {
          var eBlockSenDiv = document.getElementById(
      ...
      for (var iSen = 0; iSen < eAllSent.length; iSen++) {
        var senA = eAllSent[iSen].getAttributeNode("sn").value;
        var blockId = eAllSent[iSen].getAttributeNode("block").value;
        var eSBSDiv = document.getElementById(
          "tran_sent_" + blockId + "_" + senA
        );
        if (eSBSDiv) {
          //逐段div
...
function setWordTableVisibility() {
  ...
  document.getElementById("word_table").style.display = isVisible
    ? "block"
    : "none";
}
...
function setAllTitleVisibility(Controller, numTitle) {
  for (var i = 0; i < numTitle; i++) {
    document.getElementById("titlevisable" + numTitle).checked =
      Controller.checked;
  }
}
...
function getLocalGrammaStr_a(inStr) {
  ...
  for (i_inStr_array_0 in inStr_array_0) {
    if (inStr_array_0[i_inStr_array_0].lastIndexOf("$") != -1) {
      ...
      for (i_instr in inStr_array) {
        inStr_array[i_instr] = inStr_array[i_instr].slice(
          1,
          inStr_array[i_instr].length - 1
        );
        inStr_array[i_instr] = "。" + inStr_array[i_instr] + "。";
      }
      inStr_array_0.splice(i_inStr_array_0, 1, inStr_array.join("@"));
    } else if (
      inStr_array_0[i_inStr_array_0] !=
      "" /*&& inStr_array_0[i_inStr_array_0].lastIndexOf(\s)=-1*/
    ) {
  ...
  for (getLocalGrammaStr_i in gLocal.grammastr) {
    ...
    if (str_Gramma != "$") {
      ...
      inStr = inStr.replace(
        special_RE,
        gLocal.grammastr[getLocalGrammaStr_i].value
      );
    } else {
