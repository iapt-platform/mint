		...
		if (un.length > 0) {
			switch (item) {
				...
				case "end":
					...
					break;
				case "word":
					if (getNodeText(xAllWord[iWordIndex], "case") != ".un:begin." || getNodeText(xAllWord[iWordIndex], "case") != ".un:end." || getNodeText(xAllWord[iWordIndex], "case") != ".ctl.#.a.") {//既不是開始也不是結束
						wordid = "wb" + getNodeText(xAllWord[iWordIndex], "id");
						document.getElementById(wordid).style.display = (isVisible ? 'block' : 'none');
					}
					break;
...
function setArrange(mode) {
	...
	if (_display_para_arrange == 1) {  /* 上下对读 */
	...
	else if (_display_para_arrange == 0) { /* 0 左右对读 */
	...
	switch (language) {
		case "sinhala":
			for (get_Local_Code_Str_i in local_codestr_sinhala) {
				inStr = inStr.replace(local_codestr_sinhala[get_Local_Code_Str_i].id, local_codestr_sinhala[get_Local_Code_Str_i].value);
			}
			break;
		/*case "sc":
...
function getLocalGrammaStr(inStr) {
	let str = inStr;
	for (let i in gLocal.grammastr) {
		str = str.replace(gLocal.grammastr[i].id, gLocal.grammastr[i].value);
		//eval("inStr=inStr.replace(/"+gLocal.grammastr[getLocalGrammaStr_i].id+"/g,gLocal.grammastr[getLocalGrammaStr_i].value);");
	}
	return str;
}
...
function getLocalGrammaStr_a(inStr) {
	...
	for (i_inStr_array_0 in inStr_array_0) {
		if (inStr_array_0[i_inStr_array_0].lastIndexOf("$") != -1) {
			...
			inStr_array_0[i_inStr_array_0] = inStr_array_0[i_inStr_array_0].slice(1, inStr_array_0[i_inStr_array_0].length - 1);
...
function getLocalDictname(inStr) {
	...
	for (getLocalDictname_i in gLocal.dictname) {
		...
		LocalDictname = LocalDictname.replace(gLocal.dictname[getLocalDictname_i].id, gLocal.dictname[getLocalDictname_i].value);
	}
...
function getElementWH(element) {
	var scrW, scrH;
	if (element.innerHeight && element.scrollMaxY) {    // Mozilla    
		scrW = element.innerWidth + element.scrollMaxX;
		scrH = element.innerHeight + element.scrollMaxY;
	}
	else if (element.scrollHeight > element.offsetHeight) {    // all but IE Mac    
		scrW = element.scrollWidth;
		scrH = element.scrollHeight;
	} else if (element) { // IE Mac    
...
function tab_click(panalId, tabid, callback = null, parm = null) {
	$("#" + tabid).siblings().removeClass("act");
	$("#" + tabid).addClass("act");
	if (panalId != "") {
		$("#" + panalId).show();
		$("#" + panalId).siblings().hide();
		$("#" + panalId + "_head").show();
		$("#" + panalId + "_head").siblings().hide();
	}
	if (callback != null) {
		if (parm != null) {
			callback(parm);
