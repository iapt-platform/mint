...
class PaliTranslateService
{
    ...
    **输出范例**
    ...
    protected string $evaluatePrompt = <<<'md'
        ...
        nissaya中列出的缅文解释可能只包含巴利文的部分解释，或者只有用法格位的提示。有时是巴利文的词义的详细说明而非单词的直接意思。请根据巴利文的上下文理解nissaya的内容。只在确定nissaya和译文有明显冲突的情况下采纳nissaya的意见。如果差别不大，请不要标记。
        ...
        **span 的属性一律用单引号**（class='...'  title='...'），不要用双引号——因为 content 整体是 JSON 字符串、本身由双引号包裹，属性再用双引号极易因转义出错导致整行 JSON 解析失败、整句被丢弃。title 等属性值内若要引用文字，请使用中文全角引号「」或‘’，**严禁**出现 ASCII 双引号(")或单引号(')。
        ...
        5. 给出「建议」前必须自检：建议的译文是否与被标注的原文片段（忽略加粗符号）不同。若完全相同或者没有明显差异，说明这不构成真实问题，请不要标注，直接原样返回该片段。
