...
import type { IWbw, TFieldName, TWbwDisplayMode } from "./WbwWord"
  ...
  if (display === "list") {
    meaningInner = (
      <EditableLabel
        ...
        onKeyDown={(_event: React.KeyboardEvent<HTMLInputElement>) => {}}
        onBlur={() => {
          if (typeof onChange !== "undefined") {
            onChange(input ? input : "");
          }
    ...
    meaningInner = (
      <span
        onClick={(_event: React.MouseEvent<HTMLSpanElement, MouseEvent>) => {
          setEditable(true);
        }}
      >
        {meaning}
